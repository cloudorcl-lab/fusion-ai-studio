# Workflow Test Result: address-query-list

**Status:** PASSED
**Workflow:** XDX_SUPPLIER_CORE_20260924 (DRAFT v86232573)
**Scenario:** custom user test
**Test Intent:** Verify ordinary parent-scoped address exact-name query independently of create confirmation.
**Data Source:** file
**Evaluation:** hybrid
**Workflow Time:** 16590 ms (sum of node times)
**Test Run Time:** 18174 ms
**Test File:** `test/workflows/xdx_supplier_core_20260924/address-query-list.json`
**JSON Report:** `C:\Users\dasu\Documents\GitHub\fusion-ai-studio-1\.worktrees\xdx-supplier-core-20260924\test-reports\workflows\xdx_supplier_core_20260924\address-query-list\result.json`
**Markdown Report:** `C:\Users\dasu\Documents\GitHub\fusion-ai-studio-1\.worktrees\xdx-supplier-core-20260924\test-reports\workflows\xdx_supplier_core_20260924\address-query-list\result.md`
**HTML Report:** `C:\Users\dasu\Documents\GitHub\fusion-ai-studio-1\.worktrees\xdx-supplier-core-20260924\test-reports\workflows\xdx_supplier_core_20260924\address-query-list\result.html`

## Test Intent

**Type:** custom user test
**Intent:** Verify ordinary parent-scoped address exact-name query independently of create confirmation.

## Invocation

**Message:** `Find the address with exact AddressName XDX Core Address 20260924 for Supplier ID 300000333814409, page size 1.`
**Trigger Type:** app
**Single Turn:** no
...
| PASS | FETCH_ADDRESSES executes | Must execute | FETCH_ADDRESSES | Executed |
| PASS | SHAPE_ADDRESSES executes | Must execute | SHAPE_ADDRESSES | Executed |
| PASS | STORE_ADDRESS_LIST executes | Must execute | STORE_ADDRESS_LIST | Executed |
...
| PASS | Complete native address output | Uses a supported oraInfoDisplay widget with honest nulls and no write claim. Count, offset and continuation agree with recorded collection envelope. | Supported multiRecordWidget has count1/offset0/hasMorefalse and No next page, matching the collection; no write claim. |
...
Workflow time: 16590 ms (sum of node times)
Test run time: 18174 ms (includes CLI overhead)
...
| --- | --- | --- | --- |
| 8235 | 443 | 8678 | oci-agent/openai.gpt-5-mini |

| Node | Input Tokens | Output Tokens | Total Tokens | Model | Source |
| --- | --- | --- | --- | --- | --- |
| ADDRESS_DISPLAY | 7315 | 210 | 7525 | oci-agent/openai.gpt-5-mini | computed:inputTokens+outputTokens, nodeProgressMessage.nodeStepInfo.details[0].inputTokens, nodeProgressMessage.nodeStepInfo.details[0].modelName, nodeProgressMessage.nodeStepInfo.details[0].outputTokens |
| EXTRACT_READ | 920 | 233 | 1153 | oci-agent/openai.gpt-5-mini | computed:inputTokens+outputTokens, nodeProgressMessage.nodeStepInfo.details[0].inputTokens, nodeProgressMessage.nodeStepInfo.details[0].modelName, nodeProgressMessage.nodeStepInfo.details[0].outputTokens |
...
| --- | --- | --- | --- | --- | --- | --- |
| ADDRESS_DISPLAY | 7315 | 210 | 1 | 5 | premium | general |
| EXTRACT_READ | 920 | 233 | 1 | 5 | premium | general |

| Node | Node Time (ms) | LLM Call Time (ms) | Details |
| --- | --- | --- | --- |
| EXTRACT_READ | 8235 | 8155 | 1 runtime detail; see JSON report |
| FETCH_ADDRESS_PARENT | 46 |  | 0 runtime details; see JSON report |
| FETCH_ADDRESSES | 46 |  | 0 runtime details; see JSON report |
| ADDRESS_DISPLAY | 8263 | 7776 | 1 runtime detail; see JSON report |

| Observed Field | Values |
| --- | --- |
| modelName | "oci-agent/openai.gpt-5-mini"; "oci-agent/openai.gpt-5-mini" |
| inputTokens | 920; 7315 |
| outputTokens | 233; 210 |
...
| FETCH_ADDRESSES | BO_FUNCTION | raw | no | 1 | 618 |

## Workflow Run

**Conversation ID:** workflow-test-address-query-list-1fa73c32-95eb-4524-9422-5620b89c125b
**Job ID:** 678f25b3-d090-4bf3-970f-6d8577c27245
**Finish Reason:** stop
**Raw Event Count:** 41
...
| FETCH_ADDRESSES | NODE_START, NODE_END | yes | yes |
| SHAPE_ADDRESSES | NODE_EXECUTED | yes | yes |
| STORE_ADDRESS_LIST | NODE_EXECUTED | yes | yes |
...
- `XDX Core Address 20260924` from `FETCH_ADDRESSES`: Appears verbatim in finalOutput and comes from replayed testData.
- `LOS ANGELES` from `FETCH_ADDRESSES`: Appears verbatim in finalOutput and comes from replayed testData.
...
- `5000 CARTER DR.` from `FETCH_ADDRESSES`: Appears verbatim in finalOutput and comes from replayed testData.
- `xdx-core-address-20260924@example.invalid` from `FETCH_ADDRESSES`: Appears verbatim in finalOutput and comes from replayed testData.
