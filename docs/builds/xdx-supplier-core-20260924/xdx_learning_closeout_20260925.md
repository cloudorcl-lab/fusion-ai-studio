...
Base integration exposed Git LF/CRLF conversion as a false source-drift failure in package verification. The manifest now retains exact source/payload hashes plus a normalized text-source hash. Source comparison permits equivalent text line endings; payload inventory and byte hashes remain strict, and substantive source changes still fail. Binary files keep exact hashes. Cross-checkout installation tests cover this repair.
