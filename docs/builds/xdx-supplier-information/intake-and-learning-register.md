...
User requested all lessons be recorded for future sessions and all APIs. The documentation-only intake, evidence classifications, retrieval diagnosis, write-test design and closeout are in [api-learning-review.md](api-learning-review.md). Canonical rules are integrated into the living playbook Gate 2; earlier no-change conclusions are historical. No runtime changes or live retests are required.
