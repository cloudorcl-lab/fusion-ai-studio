...
Scope correction (2026-09-22): the user explicitly excluded every DFF, every attachment, payment behavior, third-party payment relationships and all other non-required create fields. The P0 inventory retains those resources as considered/out of scope. Enabled create payloads contain required business fields only.
...
Use one Agentic App workflow, `XDX_SUPPLIER_LIFECYCLE_AGENT`, with tool-grounded extraction, deterministic validation/state updates, non-suspending Query turns and guarded BO calls. Expose it through the real Ask Oracle entry point in Phase 1. Prove the target app's Query runtime instead of treating workflow debug as Ask Oracle acceptance. The app-bound workflow must contain no reachable `HUMAN` or `WAIT` node. Do not add a standalone web UI or Visual Builder.

Keep every Agentic App Query invocation self-contained and terminating. Carry only state that the supported app runtime demonstrably supplies between turns; otherwise require a stable business key in the current request. First implement small complete query, prepare, review and execution slices within this owner. Split into specialists only when a distinct contract justifies the dependency and its DRAFT/PUBLISHED execution boundary is proven. Agents can choose read tools and propose structured field patches; they cannot bypass the deterministic approval gate or invoke POST tools directly. Use BO_FUNCTION nodes for guarded writes.

Technology: installed `.agents/skills/aistudio`, supported bundled CLI, native workflow variables, CODE/SET_FIELDS, AGENT or structured LLM extraction, SWITCH, BO_FUNCTION and native Ask Oracle. Accuracy precedes speed; record actual model/version per model node, no speculative model sweeps.
...
| P1 — Agentic App spine, no writes | `src/workflows/xdx_supplier_lifecycle_agent.wf`; typed state, patch reducer, non-suspending Query terminal, Ask Oracle binding | Target DRAFT app Query path passes with no reachable Human/wait node; correction, cancellation and stale approval produce zero writes under supported turn-state behavior | All P1 state/guard cases plus live target-app Query evidence pass; no POST enabled |
...
| P8 — remaining children | One subphase per in-scope child in the P0 inventory, in dependency order; reuse/add BO functions only after its contract is known | Create-capable in-scope children get query, required-field review, exact approval, create and independent GET; read-only children get query and disclosed boundary | Previously completed children plus this child pass before the next; explicit exclusions and inventory coverage reconciled |
