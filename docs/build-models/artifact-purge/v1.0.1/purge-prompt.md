...
Model version: **1.0.1**. Status: template, not authorized for execution.
...
the preserved copies and selected preservation disposition before local removal. Do not run this
...
For worktree removal, require either verified integration or an explicitly authorized retained archive branch at the exact tip plus a verified history bundle outside deletion scope. Verify bundle prerequisites remain available and preserve ignored/untracked evidence separately. Record this choice during intake and reuse unchanged authorization. Archive retention does not authorize branch deletion.
