...
For every POST test, use available retained BO GET data as the starting source for candidate values. Reuse the saved samples and complete linked responses; do not rerun GET merely to prepare sample data. A GET response is not a create payload: build an explicit writable-field mapping against the exact Oracle operation documentation, request schema, field descriptions and Example Request Body.
