...
// Supplier: only the required transaction name is retained; generated keys are excluded.
let supplier = reduce('Create a supplier named XDX P3 Required Only', null);
assert.equal(supplier.state.draft.resource, 'supplier');
assert.deepEqual(supplier.state.draft.requiredFields, { Supplier: 'XDX P3 Required Only' });
...
site.queryResults = [{ SupplierSiteId: 601, SupplierSite: 'SOURCE', ProcurementBUId: 204, PaymentTerms: 'DO NOT COPY' }];
site = reduce('prepare site with SupplierSite=XDX_CHICAGO; ProcurementBUId=999', site).state;
...
// Contact: only the schema-required controlled email is retained; IDs and optional identity fields are ignored.
let contact = selectedState();
contact = reduce('prepare contact with Email=xdx-contact@example.invalid; FirstName=Ignore; SupplierContactId=777', contact);
assert.deepEqual(contact.state.draft.requiredFields, { Email: 'xdx-contact@example.invalid' });
...
assert.equal(nodes.some((node) => node.type === 'BO_FUNCTION' && /^Create/.test(node.metadata?.functionName || '')), false,
  'P3 must remain no-write');
...
console.log(JSON.stringify({
  ...
  assertions: [
    ...
    'no write node or Human/wait node'
  ]
}, null, 2));
